...
import QuickShopModal from "../QuickShopModal/QuickShopModal";
...
export default function ProductCard({ product }) {
  ...
  return (
    <>
        ...
        <div className="relative aspect-square overflow-hidden bg-gray-50">
          ...
          <div className="absolute inset-x-0 bottom-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-t from-black/50 to-transparent">
            <button 
              ...
              className="w-full bg-white text-primary font-bold py-2 rounded-lg flex items-center justify-center gap-2 hover:bg-accent hover:text-white transition-colors cursor-pointer"
            >
      ...
      {isModalOpen && (
        <QuickShopModal product={product} onClose={() => setIsModalOpen(false)} />
      )}
    </>
  );
}